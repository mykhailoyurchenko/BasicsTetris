...
void TetrisMenu::isVerticalOrientation() const {
    if (currentType == 0) { // тільки для палки
        return currentTetris[0].x == currentTetris[1].x &&
            currentTetris[1].x == currentTetris[2].x &&
            currentTetris[2].x == currentTetris[3].x;
    }
    // для інших фігур можна додати схожі перевірки за потреби
    return false;
}
