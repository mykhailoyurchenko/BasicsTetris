...
void TetrisMenu::rotateTetris() {
    ...
    // Перевірки для I-фігури (вертикальне положення)
    if (currentType == 0) {
        ...
        if (isVertical) {
            for (const auto& block : currentTetris) {
                if (block.x <= 1 || block.x >= cols - 2) {
                    return; // Забороняємо обертання біля краю у вертикальному положенні
                }
            }
        }
    }
    // Перевірки для всіх інших фігур (J, L, S, Z, T) – горизонтальне положення
    else if (currentType == 1 || currentType == 2 || currentType == 4 || currentType == 5 || currentType == 6) {
        bool isHorizontal = true;
        int y0 = currentTetris[0].y;
        for (const auto& block : currentTetris) {
            if (block.y != y0) {
                isHorizontal = false;
                break;
        ...
        if (isHorizontal) {
            for (const auto& block : currentTetris) {
                if (block.x <= 1 || block.x >= cols - 2) {
                    return; // Забороняємо обертання біля краю у горизонтальному положенні
                }
            }
        }
    }
